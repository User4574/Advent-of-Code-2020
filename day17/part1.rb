class Grid
  ...
  def iterate!
    next_grid = Array.new(@hei) { Array.new(@dep) { Array.new(@wid) { false } } }

    @hei.times { |z|
      @dep.times { |y|
        @wid.times { |x|
          s = neighbour_count(x, y, z)
          if (@grid[z][y][x] && s == 2) || s == 3
            next_grid[z][y][x] = true
          end
        }
      }
    }
